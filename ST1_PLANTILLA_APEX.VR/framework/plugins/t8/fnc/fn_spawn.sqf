...
{ 
	private [ 	"_abort", "_group", "_vehicleArray", "_posMkr", "_type", "_commArray", "_cacheArray", "_cachePos", "_PatrolMarkerArray", "_infGroup", "_groupSide", "_groupFaction", "_markerArray",
	...
	if ( count _groupArray > 2 ) then 
	{
		...
		{ 
			...
			case "SIDE":	{ _groupSide	= _groupArray param [ 2, EAST ]; };
	...
	if ( count _groupArray > 3 ) then 
	{
		...
		{ 
			...
			case "SIDE":	{ _groupSide	= _groupArray param [ 3, EAST ]; };
	...
	if ( count _groupArray > 4 ) then 
	{
		...
		{ 
			...
			case "SIDE":	{ _groupSide	= _groupArray param [ 4, EAST ]; };
	...
	switch (_groupSide) do {
		...
		case WEST: {_groupFaction = side_a_faction};
		case EAST: {_groupFaction = side_b_faction};
		case RESISTANCE: {_groupFaction = side_c_faction};
		default {_groupFaction = side_b_faction};
	};
...
// ------------------ TASK SWITCH --- UNITS WILL BE SPAWNED NOW --------------------------------------------------------------
	...
	switch ( _type ) do 
	{
		case "ATTACK": 
		{
			...
			{[_x, _groupFaction] spawn BRM_fnc_initAI;} forEach units _group;
