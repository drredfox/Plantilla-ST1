...
_orbatText = "<font size='18'>ST1</font><br/><font size='16'>Fuerzas Desplegadas</font><br/><br/><font size='14' color='#d7dbd5'>Frecuencias de Radio</font><br/><font color='#FF9E05'>NARANJA</font><font color='#9DA698'> = Frecuencia de radio de largo alcance del grupo.<br/><font color='#10D471'>VERDE</font><font color='#9DA698'> = Frecuencia de radio de onda corta del grupo.</font><br/><br/>";
{
    ...
    if (_checkin) then {
		...
		_ST1_Subgroup = parseNumber (_leader select 2);

		if (_ST1_Subgroup == 0) then {
			ST1_Group_SW_freq = _ST1_Preset select 1;
			ST1_Group_LR_freq = _ST1_Preset select 0;
		} else {
			ST1_Group_SW_freq = (_ST1_Preset select 1) + _ST1_Subgroup;
			ST1_Group_LR_freq = _ST1_Preset select 1;
		};
        _orbatText = _orbatText + format["<br/><font size='16' color='#D7DBD5'>Equipo %1</font> ",(groupID _x)];
        _orbatText = _orbatText + format["<font color='#9DA698'>[</font><font color='#FF9E05'>%1</font><font color='#9DA698'>][</font><font color='#10D471'>%2</font><font color='#9DA698'>]<br/>",ST1_Group_LR_freq,ST1_Group_SW_freq];
        {
