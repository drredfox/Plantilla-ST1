...
if !(_actualGroup) then { _unitNumber = "'"+(_unitNumber) };

_aliasACTUAL = ["CO", "Officer", "SL", "Squad Leader", "Leader"];
_aliasFTL = ["FTL", "Team Leader"];

switch (true) do {
    case (_actualGroup && _isLeader): { _role = "Lider" };
    case (_role in _aliasACTUAL): { _role = "Lider" };
    case (_role in _aliasFTL): { _role = "Lider de Equipo" };
    case (_isLeader): { _role = "Lider" };
};
