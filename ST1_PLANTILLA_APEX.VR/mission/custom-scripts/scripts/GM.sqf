...
_ADF_GM_gear = {
	...
	removeUniform _u;
	removeHeadgear _u;
	_u forceAddUniform "ST1_WDL_Uniform_light2";
	_u addHeadgear "ST1_Beret";
	if (ADF_mod_TFAR) then {_u addBackpack "tf_rt1523g";} else {_u addBackpack "B_TacticalPack_blk";};
	_u addVest "ST1_WDL_Plate_Carrier_H";
	...
	if (ADF_Clan_uniformInsignia) then {[_u,"ST1"] call BIS_fnc_setUnitInsignia};
};

if !(isNil "GM_1") then {
	if !(player == GM_1) exitWith {};
	GM_1 addAction ["<t align='left' color='#FBF4DF'>GM - Teleport</t>",{openMap true;hintSilent format ["%1, pulsa en mapa para teletransporte...", name vehicle player];onMapSingleClick "vehicle player setPos _pos; onMapSingleClick ' '; true; openMap false; hint format ['%1, estás en: %2', name vehicle player, getPosATL player];";},[],-85, true, true,"", ""];
...
if !(isNil "GM_2") then {
	...
	GM_2 addAction ["<t align='left' color='#FBF4DF'>GM - Teleport</t>",{openMap true;hintSilent format ["%1, pulsa en mapa para teletransporte...", name vehicle player];onMapSingleClick "vehicle player setPos _pos; onMapSingleClick ' '; true; openMap false; hint format ['%1, estás en: %2', name vehicle player, getPosATL player];";},[],-85, true, true,"", ""];
